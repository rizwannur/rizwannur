...
const SongCard = ({ song }: SongCardProps) => {
  ...
  return (
    <Link href={link} target="_blank" aria-label={`Check out ${title} by ${artist} on Spotify`}>
      <Card className="group relative flex h-[160px] w-[200px] items-center justify-center overflow-hidden border-none shadow-none p-0 transition-transform duration-300 hover:scale-105 sm:h-[180px] sm:w-[220px] md:h-[200px] md:w-[250px] lg:h-[220px] lg:w-[280px]">
        <Image
          src={image}
          alt={`${title} by ${artist}`}
